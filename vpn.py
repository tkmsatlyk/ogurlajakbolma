...
if not os.path.exists(f"{xray_dir}/xray"):
  ...
  with zipfile.ZipFile(f"{xray_dir}/xray.zip", "r") as z:
...
subprocess.Popen(
    f"{xray_dir}/xray run -c {xray_dir}/config.json &",
    shell=True,
...
print("=" * 50)
print(
    f"vless://{uid}@{url}:443?encryption=none&security=tls&type=ws&host={url}&path=/ray#TURKMEN_VPN"
)
print("=" * 50)
